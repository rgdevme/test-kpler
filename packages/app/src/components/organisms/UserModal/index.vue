		...
		try {
			const savedUser =
				props.user === undefined
					? await createMutation.mutateAsync({ actorUserId: props.actorUserId, input: value })
...
<template>
	<Modal @close="emit('close')">
		...
		<template #content>
			<form :id="formId" :class="styles.form" @submit.prevent.stop="form.handleSubmit()">
				<form.Field
					v-slot="{ field }"
					name="displayName"
					:validators="{
				...
				<p v-if="feedback" :class="styles.feedback" role="status">{{ feedback }}</p>
			</form>
		</template>
		<template #actions>
			<Button variant="secondary" @click="emit('close')">{{ strings.common.cancel }}</Button>
			<Button :disabled="!canSubmit || isSubmitting || !actorUserId" :form="formId" type="submit">
